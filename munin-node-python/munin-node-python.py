...
class pymunin:
        ...
        for k in self.pluginconf:
            ...
            if r is not None:
                ...
                if "env" in conf:
                    for e in conf["env"]:
                        env[e] = conf["env"][e]
        print(f"DEBUG: run {plugin} as {user}:{group}")
        print(env)
        ex = [f"/etc/munin/plugins/{plugin}"]
        if options:
            ex.append(options)
    ...
    def parse(self, C):
        ...
        if buf[:4] == 'cap ':
            ...
            return
        if buf[:4] == 'list':
            dirFiles = os.listdir("/etc/munin/plugins")
        ...
        if buf[:7] == 'config ':
            ...
            if len(tokens) != 2:
                ...
                return
            plugin = tokens[1]
            data = self.exec_plugin(plugin, "config")
        ...
        if buf[:6] == 'fetch ':
            ...
            if len(tokens) != 2:
                ...
                return
            plugin = tokens[1]
            data = self.exec_plugin(plugin, None)
            s.send(data.encode("UTF8"))
            return
        if buf == "quit":
            s.close()
...
def main():
    ...
    parser.add_argument("--logdir", help="Override xython log directory", default="/var/log/pymunin/")
